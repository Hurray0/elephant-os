...
/* 用转义字符定义部分控制字符 */
#define esc '\033'	// 八进制表示字符, 也可以写成'\x1b'
#define backspace '\b'
#define tab '\t'
#define enter '\r'
#define delete '\177'  // 八进制表示字符, 也可以写成'\x7f'
...
static void intr_keyboard_handler(void) {
    ...
    if (break_code) {	// 若是断码
        /* 由于ctrl_r和alt_r的make_code是两个字节, 所以可通过收集
         * 两次断码来判断此断码是否为ctrl_r和alt_r */
        ...
        uint8_t index = (scancode &= 0x00ff);	// 得到一维数组的索引
        char cur_char = keymap[index][shift];	// 在数组中找到对应的字符

        /* 只处理ascii码不为0的键 */
        if (cur_char) {
            // put_char(cur_char);
            /* 若 kbd_buf 中未满,则将当前扫描码加入缓冲区 */
            if (!ioq_full(&kbd_buf)) {
        ...
        if (scancode == ctrl_l_make || scancode == ctrl_r_make) {
            ...
            /* 不同的是, caps_lock不是记录是否按下, 而是记录是否开启了大写锁定 */
