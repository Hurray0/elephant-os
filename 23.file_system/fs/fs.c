...
static bool mount_partition(struct list_elem *pelem, int arg) {
  ...
  if (!strcmp(part->name, part_name)) {
    ...
    for (uint32_t i = 0; i < sb_buf->block_bitmap_sects; i++) {
      ide_read(hd, sb_buf->block_bitmap_lba + i, cur_part->block_bitmap.bits + i * 512, 1);
    }
    /*************************************************************/
    ...
    for (uint32_t i = 0; i < sb_buf->inode_bitmap_sects; i++) {
      ide_read(hd, sb_buf->inode_bitmap_lba + i, cur_part->inode_bitmap.bits + i * 512, 1);
    }
    /*************************************************************/
...
static void partition_format(struct partition *part) {
  ...
  uint32_t inode_bitmap_sects =
      DIV_ROUND_UP(MAX_FILES_PER_PART,
  ...
  // 为避免`qemu`/`virtualbox`等虚拟机不支持一次性写入这么大的数据,
  // 故采用分批写入的策略
  for (uint32_t i = 0; i < sb.block_bitmap_sects; i++) {
    ide_write(hd, sb.block_bitmap_lba + i, buf + i * 512, 1);
  }
  ...
  // 为避免`qemu`/`virtualbox`等虚拟机不支持一次性写入这么大的数据,
  // 故采用分批写入的策略
  for (uint32_t i = 0; i < sb.inode_table_sects; i++) {
    ide_write(hd, sb.inode_table_lba + i, buf + i * 512, 1);
  }
