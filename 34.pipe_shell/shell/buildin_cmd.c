...
static void wash_path(char *old_abs_path, char *new_abs_path) {
  ...
  while (name[0]) {
    ...
    if (!strcmp("..", name)) {
      ...
      if (slash_ptr !=
          new_abs_path) { // 如new_abs_path为“/a/b”,".."之后则变为“/a”
        *slash_ptr = 0;
        ...
        *(slash_ptr + 1) = 0;
                      ...
                      name)) { // 如果路径不是‘.’,就将name拼接到new_abs_path
      if (strcmp(
              new_abs_path,
...
void make_clear_abs_path(char *path, char *final_path) {
  ...
  if (path[0] != '/') { // 若输入的不是绝对路径,就拼接成绝对路径
    ...
    if (getcwd(abs_path, MAX_PATH_LEN) != NULL) {
      if (!((abs_path[0] == '/') &&
            (abs_path[1] == 0))) { // 若abs_path表示的当前目录不是根目录/
        strcat(abs_path, "/");
      }
    }
  }
...
void buildin_ls(uint32_t argc, char **argv) {
  ...
  if (pathname == NULL) { // 若只输入了ls 或 ls
                          // -l,没有输入操作路径,默认以当前路径的绝对路径为参数.
    if (NULL != getcwd(final_path, MAX_PATH_LEN)) {
      pathname = final_path;
    } else {
